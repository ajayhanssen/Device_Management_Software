...
with (col1):
    ...
    with tab1:
        ...
        with container:
            ...
            dev_attributes = pd.DataFrame(
                columns=["Gerätenummer", "Verantwortlicher", "Erstelldatum", "Zuletzt bearbeitet"])
            ...
            dev_attributes.loc[len(dev_attributes.index)] = [selected_device.id,
                                                             selected_device_user.name if selected_device_user is not None
                                                             else "Kein Verantwortlicher",
                                                             selected_device.creation_date.strftime("%d.%m.%Y"),
                                                             selected_device.last_update.strftime("%d.%m.%Y")]
            ...
            with st.expander("Gerät bearbeiten"):
                ...
                with st.form(key="edit_device_form", border=False):
                    new_name = st.text_input("Gerätename", key="edit_name_device", placeholder="Name eingeben",
                    ...
                    new_user = st.selectbox("Verantwortlicher", options=[user.name for user in users_dict.values()],
                                            key="selectbox_edit_device_user", index=None,
                                            placeholder="Neuen Verantwortlichen auswählen")
        ...
        with st.expander("Neues Gerät hinzufügen"):
            ...
            with st.form(key="add_device_form", clear_on_submit=True, border=False):
                ...
                new_dev_id = st.number_input("Gerätenummer *", key="id_new_device", placeholder="ID eingeben",
                                             min_value=0)
                new_dev_user = st.selectbox("Verantwortlicher", options=[user.name for user in users_dict.values()],
                                            key="selectbox_new_device_user", index=None,
        ...
        for key in users_dict:
            with st.expander(F"{users_dict[key].name} ({users_dict[key].id})"):
                with st.form(key=F"edit_user_form_{users_dict[key].id}", border=False):
                    ...
                    new_name = st.text_input("Nutzername", key=F"edit_name_user_{users_dict[key].id}",
                                             placeholder="Name eingeben", value=users_dict[key].name)
                    new_id = st.text_input("Nutzer-ID", key=F"edit_id_user_{users_dict[key].id}",
                                           placeholder="ID eingeben", value=users_dict[key].id)
    ...
    with tab3:
        ...
        sel_dev = st.selectbox("Gerät auswählen", options=[device.name for device in devices_dict.values()],
                               key="mtn_selectbox_device", index=None, placeholder="Gerät auswählen")
            ...
            if sel_dev_mtn is None:
                ...
                with st.expander("Neuen Wartungsplan hinzufügen"):
                    with st.form(key="add_mtn_form", clear_on_submit=False, border=False):
                        ...
                        new_mtn_int = st.number_input("Wartungsintervall (Tage)", key="new_mtn_interval", min_value=1,
                                                      value=1)
                        new_mtn_cost = st.number_input("Kosten einer Wartung", key="new_mtn_cost", min_value=0.0)
                        new_mtn_plan = MTN_Plan(new_mtn_int, new_mtn_start, new_mtn_cost, new_mtn_last, new_mtn_end,
                                                sel_dev.id, None)
                        if st.form_submit_button("Hinzufügen"):
                ...
                mtn_df = pd.DataFrame(
                    columns=["Erste Wartung", "Letzte Wartung", "Nächste Wartung", "Kosten einer Wartung",
                             "Wartungsintervall", "Ende der Lebensdauer"])
                mtn_df.loc[len(mtn_df.index)] = [sel_dev_mtn.first_mtn.strftime('%d.%m.%Y'),
                                                 sel_dev_mtn.last_mtn.strftime('%d.%m.%Y'),
                                                 sel_dev_mtn.next_mtn.strftime('%d.%m.%Y'), sel_dev_mtn.mtn_cost,
                                                 sel_dev_mtn.mtn_int, sel_dev_mtn.end_of_life.strftime('%d.%m.%Y')]
                ...
                with st.expander("Wartungsplan bearbeiten"):
                    with st.form(key="edit_mtn_form", clear_on_submit=False, border=False):
                        new_mtn_start = st.date_input("Erste Wartung", key="new_mtn_start", format="DD.MM.YYYY",
                                                      value=sel_dev_mtn.first_mtn)
                        new_mtn_last = st.date_input("Zuletzt gewartet", key="new_mtn_last", format="DD.MM.YYYY",
                                                     value=sel_dev_mtn.last_mtn)
                        new_mtn_end = st.date_input("Ende der Lebensdauer", key="new_mtn_end", format="DD.MM.YYYY",
                                                    value=sel_dev_mtn.end_of_life)

                        next_mtn_string = sel_dev_mtn.next_mtn.strftime("%d.%m.%Y")
                        new_next = st.date_input(F"Nächsten Wartungstermin ändern (geplant: {next_mtn_string})",
                                                 key="new_mtn_next", format="DD.MM.YYYY", value=sel_dev_mtn.next_mtn)

                        new_interval = st.number_input("Wartungsintervall (Tage)", key="new_mtn_interval", min_value=1,
                                                       value=sel_dev_mtn.mtn_int)
                        new_cost = st.number_input("Kosten einer Wartung", key="new_mtn_cost",
                                                   value=sel_dev_mtn.mtn_cost, min_value=0.0)

                        if st.form_submit_button("Speichern"):
                            if any(arg is None for arg in
                                   [new_mtn_start, new_mtn_last, new_mtn_end, new_interval, new_cost, new_next]):
    ...
    with tab4:
        ...
        sel_dev = st.selectbox("Gerät auswählen", options=[device.name for device in devices_dict.values()],
                               key="selectbox_device", index=None, placeholder="Gerät auswählen")
        if sel_dev is not None:
            ...
            if sel_dev_reservations != []:
                for reservation in sel_dev_reservations:
                    ...
                    current_reservations.loc[len(current_reservations.index)] = [res_user.name,
                                                                                 reservation.res_start.strftime(
                                                                                     '%d.%m.%Y %H:%M'),
                                                                                 reservation.res_end.strftime(
                                                                                     '%d.%m.%Y %H:%M')]
                ...
                if sel_dev_reservations != []:
                    with st.expander("Reservierung löschen"):
                        with st.form(key="delete_reservation_form", clear_on_submit=True, border=False):
                            if sel_dev_reservations != []:
                                reserv_index = st.number_input("Index der Reservierung", key="reserv_index",
                                                               min_value=0, max_value=len(sel_dev_reservations) - 1,
                                                               value=0)
                                if st.form_submit_button("Löschen"):
                                    sel_dev_reservations[reserv_index].delete_reservation(
                                        sel_dev_reservations[reserv_index].doc_index)
            ...
            with st.expander("Neue Reservierung hinzufügen"):
                with st.form(key="add_new_reservation_form", clear_on_submit=False, border=False):
                    ...
                    new_res_user = st.selectbox("Nutzer auswählen", options=[user.name for user in users_dict.values()],
                                                key="selectbox_new_res_user", index=None,
                                                placeholder="Nutzer auswählen")

                    is_available = True
                    for reserv in sel_dev_reservations:
                        if reserv.res_start <= datetime.combine(res_start_date,
                                                                res_start_time) <= reserv.res_end or reserv.res_start <= datetime.combine(
                                res_end_date, res_end_time) <= reserv.res_end:
                            is_available = False
...
with col2:
    ...
    for reservation in all_reservations:
        if reservation.res_start <= datetime.now() <= reservation.res_end:
            ...
            active_reservations.loc[len(active_reservations.index)] = [reservation_device.name, reservation_user.name,
